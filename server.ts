...
async function startServer() {
  ...
  app.get("/api/winners", (req, res) => {
    const winners = db.prepare("SELECT name, store, cpf FROM registrations WHERE is_winner = 1 ORDER BY created_at DESC").all();
    res.json(winners);
  });
  ...
  app.post("/api/register", (req, res) => {
    try {
      ...
      const winner = db.prepare("SELECT * FROM registrations WHERE cpf = ? AND is_winner = 1").get(cpf);
      if (winner) {
        return res.status(403).json({ error: "Você já foi premiado!" });
      }

      // Winning logic: 1 winner per week per store, accumulating
